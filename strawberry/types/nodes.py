...
Call `SelectionField` on a graphql `FieldNode`, such as in `info.field_nodes`.
...
Selection = Union["SelectionField", "FragmentSpread", "InlineFragment"]
...
def selection(node: Node) -> Selection:
    ...
    if hasattr(node, "alias"):
        return SelectionField(node)  # type: ignore
...
@dataclasses.dataclass
class SelectionField(FragmentSpread):
